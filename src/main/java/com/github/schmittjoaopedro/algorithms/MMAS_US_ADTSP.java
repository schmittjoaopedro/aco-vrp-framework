...
public class MMAS_US_ADTSP implements Runnable {
    ...
    public void executeLocalSearch() {
        ...
        usOperator.init(graph, getVertexTour(iterationBest), iterationBest.getCost());
        usOperator.optimize();
        List<Vertex> resultRoute = usOperator.getResult();
        for (int j = 0; j <= graph.getVertexCount(); j++) {
            iterationBest.getTour()[j] = resultRoute.get(j).getId();
        }
